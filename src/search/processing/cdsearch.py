...
def prepare_cdsearch_output_paths(output_dir: Path) -> Dict[str, Path]:
    ...
    return {
        ...
        "default_top1_detailed": output_dir / "cdsearch_default_top1_detailed.tsv",
...
def write_cdsearch_result_tables(
    *,
    ...
    - cdsearch_default_top1_detailed.tsv

    The default top1 table is a minimal editable table. Users may manually edit
    or replace it before running the cdsearch_extract stage.
    """
    ...
    default_top1_detailed = select_default_top1_detailed(df)
    default_top1_detailed.to_csv(
        output_paths["default_top1_detailed"],
        sep="\t",
        index=False,
    )

    default_top1 = build_default_top1_selection_table(default_top1_detailed)
    default_top1.to_csv(
...
def select_default_top1_detailed(df: pd.DataFrame) -> pd.DataFrame:
    ...
    This detailed table is intended for inspection and audit, not manual editing.
    ...
    top1.insert(
        2,
        "selection_method",
        "default_bitscore_desc_evalue_asc",
    )

    top1 = top1.rename(
        columns={
            "PSSM_ID": "selected_PSSM_ID",
        }
    )
    ...
    return top1[output_cols]  # type: ignore


def build_default_top1_selection_table(
    default_top1_detailed: pd.DataFrame,
) -> pd.DataFrame:
    """
    Build a minimal editable default top1 selection table.

    This table is intended to be used directly by the cdsearch_extract stage.
    Only two columns are required:

    - query_id
    - selected_PSSM_ID
    """
    return default_top1_detailed[
        [
            "query_id",
            "selected_PSSM_ID",
        ]
...
def build_selected_domain_hits(
    *,
    ...
    The selected top1 table must contain:

    - query_id
    - selected_PSSM_ID

    Each selected query_id / selected_PSSM_ID pair must exist in the
    CD-Search all-hits table.
    """
    required_selected_cols = {"query_id", "selected_PSSM_ID"}
    missing_cols = required_selected_cols - set(selected_df.columns)

    if missing_cols:
        raise ValueError(
            "Selected top1 table is missing required columns: "
            f"{sorted(missing_cols)}"
        )

    required_hit_cols = {"query_id", "PSSM_ID"}
    ...
    selected_df = selected_df[["query_id", "selected_PSSM_ID"]].copy()

    selected_df["query_id"] = selected_df["query_id"].astype(str)
    selected_df["selected_PSSM_ID"] = selected_df["selected_PSSM_ID"].astype(str)
    ...
    duplicated = selected_df[selected_df.duplicated(subset=["query_id"], keep=False)]
    ...
    selected_hits = all_hits_df.merge(
        selected_df,
        left_on=["query_id", "PSSM_ID"],
        right_on=["query_id", "selected_PSSM_ID"],
        how="inner",
    )

    missing_selection = selected_df.merge(
        selected_hits[["query_id", "PSSM_ID"]],
        left_on=["query_id", "selected_PSSM_ID"],
        right_on=["query_id", "PSSM_ID"],
        how="left",
        indicator=True,
    )
    ...
    if not missing_selection.empty:
        ...
        raise ValueError(
            "Some selected query_id / selected_PSSM_ID pairs were not found "
            f"in the all-hits table. See: {missing_path}"
        )

    selected_hits = selected_hits.drop(columns=["selected_PSSM_ID"])
...
def print_cdsearch_summary(
    *,
    ...
    print(f"🔎 Default top1 details   : {output_paths['default_top1_detailed']}")
    ...
    print("─" * 70)
    print(
        "Inspect the default top1 proposal or manually edit a selected top1 table, "
        "then run the cdsearch_extract stage to generate alignment blocks and "
        "domain FASTA files.\n"
    )
